import React from 'react';
import "../Stylesheet/Footer.css"

function Footer() {
  return (
      <>
      <div className='f-main'>
           <img className='f-img' src="data:image/png;base64," alt="logo"/>
           <div className='f-genel'>
               <ul>
                   <h1>Genel</h1>
                   <li>Kayıt Olun</li>
                   <li>Yardım Merkezi</li>
                   <li>Hakkında</li>
                   <li>Basın</li>
                   <li>Blog</li>
                   <li>Kariyer</li>
                   <li>Geliştiriciler</li>
               </ul>
           </div>
           <div className='f-göz'>
               <ul>
                   <h1>LinkedIn'e Göz Atın</h1>
                   <li>Learning</li>
                   <li>İş İlanları</li>
                   <li>Maaş</li>
                   <li>Mobil</li>
                   <li>Hizmetler</li>
                   <li>Ürünler</li>
               </ul>
           </div>
           <div className='f-iş'>
               <ul>
                   <h1>İşletme Çözümleri</h1>
                   <li>Yetenek</li>
                   <li>Pazarlama</li>
                   <li>Satış</li>
                   <li>Learning</li>
               </ul>
           </div>
           <div className='f-dizin'>
               <ul>
                   <h1>Dizinler</h1>
                   <li>Üyeler</li>
                   <li>İş İlanları</li>
                   <li>Şirketler</li>
                   <li>Maaşlar</li>
                   <li>Sergilenen</li>
                   <li>Learning</li>
                   <li>Gönderiler</li>
                   <li>Yazılar</li>
                   <li>Okullar</li>
                   <li>Haberler</li>
                   <li>Haber Bültenleri</li>
                   <li>Ürünler</li>
                   <li>İçerik Konuları</li>
               </ul>
           </div>
      </div>
      <div className='f-f'>
          This project purpose is totally for clone and no income expectation
      </div>
      </>
  );
}

export default Footer;
